...
The dashboard is a standard React application built with Vite.
...
Build the production bundle with `npm run build`, then serve it locally with `npm run preview`.

The app uses `/` and `/console`. Configure a history-API fallback to `index.html` when deploying to static hosting so direct visits to `/console` load correctly.
